...
TEST_F(ParserSuite, ParseDeclaratorListWithArrayLast) {
  char inputstr[] = "int a, b[4][];";
  ASSERT_THAT(input_parsing_successful(&parser, inputstr), IsTrue());
  EXPECT_THAT(StdoutMatches("b is a(n) array of 4 and"), IsTrue());
  EXPECT_THAT(StdoutMatches(" a is a(n) int"), IsTrue());
}
